...
from common.config import ALLURE_TESTCASES, ALLURE_SUMMARY
from common.utils.dir_control import ensure_path_sep
from common.utils.models import TestMetrics
...
class AllureFileClean:
    ...
    def get_testcases(cls) -> List:
        ...
        # 将所有数据都收集到files中
        files = []
        for i in get_all_caseyaml(ensure_path_sep(ALLURE_TESTCASES), is_yaml=False):
    ...
    def get_case_count(cls) -> "TestMetrics":
        """ 统计用例数量 """
        try:
            file_name = ensure_path_sep(ALLURE_SUMMARY)
...
if __name__ == '__main__':
    ...
    SendReport(AllureFileClean().get_case_count()).send()
